...
@property (nonatomic, assign) YLBIndicatorType indicatorType;

@property (nonatomic, strong) NSArray *titlesArr;

@end

@implementation YLBSegmentTitleView
...
- (void)layoutSubviews
{
    ...
    if (_titleFont != _titleSelectFont) {
        for (int idx = 0; idx < self.titlesArr.count; idx++) {
            ...
            CGFloat itemBtnWidth = [YLBSegmentTitleView getWidthWithString:self.titlesArr[idx] font:titleFont] + self.itemMargin;
    ...
    else
    {
        for (NSString *title in self.titlesArr) {
            CGFloat itemBtnWidth = [YLBSegmentTitleView getWidthWithString:title font:titleFont] + self.itemMargin;
            totalBtnWidth += itemBtnWidth;
        }
    }
...
- (void)moveIndicatorView:(BOOL)animated
{
    ...
    CGFloat indicatorWidth = [YLBSegmentTitleView getWidthWithString:self.titlesArr[self.selectIndex] font:titleFont];
    [UIView animateWithDuration:(animated?0.05:0) animations:^{
        switch (self.indicatorType) {
            case YLBIndicatorTypeDefault:
                ...
                break;
            case YLBIndicatorTypeEqualTitle:
...
- (void)btnClick:(UIButton *)btn
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(YLBSegmentTitleView:startIndex:endIndex:)]) {
        [self.delegate YLBSegmentTitleView:self startIndex:self.selectIndex endIndex:index];
    }
